...
const SkinTone = () => {
  ...
  // Function to submit the image for skin tone prediction
  // Function to submit the image for skin tone prediction
const handleSubmit = async () => {
    ...
    try {
      const response = await axios.post('http://localhost:5000/skin-tone', formData, {
        headers: {
          'Content-Type': 'multipart/form-data',
        },
      });
      setResult(response.data); // Updated to use setResult
    } catch (error) {
      if (error.response) {
  ...
  return (
    <div>
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: 20 , marginTop: '50px'}}>
        ...
        <h2 className='mt-4 text-center'>SKIN-TONE DETECTION</h2>
